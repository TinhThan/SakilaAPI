...
namespace SakilaAPI.Core.CQRS.User.CommandHandler
{
    ...
    {
        public NotifycationHub notifyHub { get; set; }
        public LoginCommandHandler(DataContext dataContext, IMapper mapper, NotifycationHub notifycationHub) : base(dataContext, mapper)
        {
            notifyHub = notifycationHub;
        }
        ...
        public async Task<AccessTokenResponse> Handle(LoginCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (resultUpdate > 0)
                {
                    ...
                    await notifyHub.SendMessage("Login", userExists.UserName);
                }
