namespace SakilaAPI.Core.Contants
{
    public static class MessageSystem
    {
        public const string NO_DATA = "Không có dữ liệu.";
        public const string SERVER_ERROR = "Đã xảy ra lỗi ở hệ thống.";
        public const string DATA_INVALID = "Thông tin không hợp lệ.";
        public const string VERSION_UPDATE = "Version_Update";
        public const string ADD_SUCCESS = "Add_Success";
        public const string ADD_FAIL = "Add_Fail";
        public const string UPDATE_SUCCESS = "Update_Success";
        public const string UPDATE_FAIL = "Update_Fail";

        public const string LOGIN_FAIL = "Login_Fail";

        public const string msgAUTH_AUTHENTICATED_FAIL = "Chứng thực không thành công.";
        public const string AUTH_AUTHENTICATED_ERROR = "Auth_Authenticated_Error";
        public const string AUTH_NOT_PERMISSION = "Auth_Not_Permission";
        public const string TOKEN_NOT_FOUND = "Token_Not_Found";

        public const string AUTH_FORBIDDEN = "Forbidden";
        public const string AUTH_NOT_AUTHENTICATED = "Auth_Not_Authenticated";
        public const string AUTH_AUTHENTICATED_ACCEPT = "Auth_Authenticated_Accept";
        public const string AUTH_AUTHENTICATED_DENY = "Auth_Authenticated_Deny";
        public const string AUTH_AUTHENTICATED_VALIDATED = "Auth_Authenticated_Validated";

        public const string REFRESH_TOKEN_NOT_FOUND = "Refresh_Token_Not_Found";
        public const string REFRESH_TOKEN_INVALID = "Refresh_Token_Invalid";
        public const string REFRESH_TOKEN_EXPIRED = "Refresh_Token_Expired";

        public const string TIME_NOT_FOUND = "Time_Not_Found";
        public const string TOKEN_EXPIRED = "Token_Expired";
        public const string TOKEN_INVALID = "Token_Invalid";

        public const string Anonymous = "Anonymous";

        public const string USERNAME_ISNOT_NUMBERPHONE = "UserName_IsNot_NumberPhone";
        public const string USERNAME_IS_EMPTY = "UserName_Is_Empty";
        public const string PASSWORD_IS_EMPTY = "Password_Is_Empty";

        public const string msgAUTH_FORBIDDEN = "Tài khoản không được phép truy cập.";
        public const string msgAUTH_AUTHENTICATED_VERIFICATION = "Xác minh chứng thực không thành công.";
        public const string msgSOMETHING_DATA_INVALID = "Thông tin không hợp lệ.";
        public const string msgNO_DATA = "Không có dữ liệu.";
        public const string msgINTERNAL_SERVER_ERROR = "Đã xảy ra lỗi ở hệ thống.";
    }
}
